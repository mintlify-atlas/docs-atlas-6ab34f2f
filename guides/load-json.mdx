...
<Note>
  Obstacle type strings are **case-insensitive**. The controller normalizes the first character to uppercase before creating each `Obstacle` model instance (e.g. `"cone"` becomes `"Cone"`, `"ROCK"` becomes `"Rock"`). All other characters are preserved as-written.
</Note>
...
<Steps>
  ...
  <Step title="Review the preview">
    The textarea shows the full JSON content. Verify that the obstacle list and `total_distance` look correct. The textarea is populated automatically and is not editable through the UI — it reflects exactly what will be sent to the backend.
  </Step>
...
| Condition | Behaviour |
|-----------|-----------|
| Textarea is empty or has fewer than 50 characters | Toast: *No data to send!* — request is not made |
