...
`RoadController` and `AVLController` receive a reference to the shared `TreeService` instance so all socket communication funnels through the same connection.
