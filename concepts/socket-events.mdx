...
<Note>
  The `connected`, `disconnected`, and `obstacles_inserted` events are handled silently — `connected` and `disconnected` each log a message to the console, while `obstacles_inserted` has an empty handler. None of the three dispatch DOM events or are consumed by any controller.
</Note>
